...
public class CepEventMappingPage extends BasePage {
	...
	@FindBy(xpath="//i[@class='fas fa-code']")
	private WebElement CEPImg;
	...
	@FindBy(xpath="//tbody/tr/td/p[@class='k-reset']/../../following-sibling::tr/td[8]")
	private WebElement groupbyTransferFlag;
		
	@FindBy(xpath="//a[text()='Transfer Flag']")
	private WebElement TransferFlag;
	...
	@FindBy(id="EventType")
	private WebElement CepEventTextbox;
	
	@FindBy(id="Description")
	private WebElement DescriptionTextbox;
	
	@FindBy(id="ProductType")
	private WebElement ProductTypeTextbox;
	
	@FindBy(id="Bin")
	private WebElement BinsTextbox;
	
	@FindBy(css="span[aria-owns='TransferStatus_listbox']")
	private WebElement TransferFlagDropdown;
	
	@FindBy(css="ul[id='TransferStatus_listbox'] li")
	private List<WebElement> TransferFlagListbox;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
		    if (f.getName().startsWith("CEP Event Mapping")) {
		        f.delete();
		    }
		}
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "CEP Event Mapping");
		return Status;
	}
    ...
    public boolean groupby() {
		DragandDrop(TransferFlag,droptarget);
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		if(groupby.getText().split(": ")[1].equals(groupbyTransferFlag.getText()))
		{return true;}
		else
			return false;		
	}
	...
	public boolean ExporttoExcelWithoutData(CepEventMappingDetails cepEventMappingDetails ) throws Exception {
		searchCepEventMapping(cepEventMappingDetails.getCepEvent());
		waitForJqueryLoad(driver);
		Thread.sleep(1000);
		selectWebElement(exporttoexcel);
	...
	public void addNewCepEventMappingRecord(CepEventMappingDetails details) throws Exception {
		...
		enterValueToTxtField(BinsTextbox,details.getBins());
		selectWebElement(TransferFlagDropdown);
        selectDropdownFromVisibleText(TransferFlagListbox,details.getTransferFlag());
