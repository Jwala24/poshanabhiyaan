...
public class IntentMappingPage extends BasePage{
	...
	@FindBy(css="span[aria-owns='Product_listbox']")
	private WebElement ProductDropdown;
	
	@FindBy(css="ul[id='Product_listbox'] li")
	private List<WebElement> ProductListbox;
	
	@FindBy(css="span[aria-owns='Segment_listbox']")
	private WebElement SegmentDropdown;
	
	@FindBy(css="ul[id='Segment_listbox'] li")
	private List<WebElement> SegmentListbox;
	...
	@FindBy(css="span[aria-owns='IntentTalent_listbox']")
	private WebElement IntentTalentDropdown;
	
	@FindBy(css="ul[id='IntentTalent_listbox'] li")
	private List<WebElement> IntentTalentListbox;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
		    if (f.getName().startsWith("Intent Mapping")) {
		        f.delete();
		    }
		}
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "Intent Mapping");
		return Status;
	}
	...
	public boolean clearAll(IntentMappingDetails details) throws Exception {
        ...
        enterValueToTxtField(searchTextBox,details.getVDN());
	...
	public boolean ExporttoExcelWithoutData(IntentMappingDetails details ) throws Exception {
		searchIntentMappingRecord(details.getVDN());
		waitForJqueryLoad(driver);
		Thread.sleep(1000);
		selectWebElement(exporttoexcel);
